...
import CustomLabelModal from '../leftovers/CustomLabelModal';
...
const LeftoverTensor = ({ setFormDataProp, setFoodImage }) => {
  ...
  async function populateForm(label) {
    ...
    await canvas.current.toBlob(blob => {
      ...
      let file = new File([blob], label.toLowerCase() + '.png', {
        type: 'image/png',
      });
  ...
  async function runPrediction() {
    ...
    setPredictions(
      predictedLabels.map((val, i) => ({
        val: val,
        text: `${val} ${Math.round(predictedProbs[i] * 100)}%`,
      }))
    );
    ...
    <Container>
      ...
      >
        ...
        {predictions
          ...
          : 'NO PREDICTIONS YET'}
        {predictions ? (
          <CustomLabelModal callBack={populateForm} imageData={imageData} />
        ) : (
          ''
        )}
      </div>
